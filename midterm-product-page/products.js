...
function loadData() {
  ...
  for (let p in products) {
    ...
    for (let up in uniqueProducts) {
      if (
        products[p].item === uniqueProducts[up].item &&
        products[p].ppu === uniqueProducts[up].ppu
      ) {
        uniqueProducts[up].discount =
          parseInt(uniqueProducts[up].discount) +
          parseInt(products[p].discount);
        uniqueProducts[up].quantity =
          parseInt(uniqueProducts[up].quantity) +
          parseInt(products[p].quantity);
        isDuplicate = true;
        break;
      }
    }
  ...
  for (let p in products) {
    ...
    let cellQuantity =
      '<td style="text-align: right;">' + products[p].quantity + "</td>";
    let cellItem =
      '<td style="text-align: left;">' + products[p].item + "</td>";
    let cellPPU = '<td style="text-align: right;">' + products[p].ppu + "</td>";
    let cellDiscount =
      '<td style="text-align: right;">' + products[p].discount + "</td>";
...
function clearData() {
  if (
    !confirm(
      "Are you sure you want to remove all the item in your cart? This action can't be undone"
    )
  ) {
    return;
  }
